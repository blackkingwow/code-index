...
void cbm_index_supervisor_set_binary_path(const char *path) {
    if (path && path[0]) {
...
int cbm_index_spawn_worker(const char *args_json, bool single_thread, const char *marker_file,
    ...
    char self[1024] = {0};
    snprintf(self, sizeof(self), "%s", g_binary_path);
    if (!self[0]) {
    ...
    FILE *af = cbm_fopen(args_path, "wb");
    if (!af) {
        cbm_log_warn("index.supervisor.args_write_failed", "action", "degrade_in_process");
        return -1;
    }
    (void)fputs(args_json ? args_json : "{}", af);
    (void)fclose(af);
